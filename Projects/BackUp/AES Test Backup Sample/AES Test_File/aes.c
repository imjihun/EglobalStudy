...
static void BlockCopy_n(uint8_t* output, const uint8_t* input, int size)
{
  uint8_t i;
  for (i=0;i<size;++i)
  {
...
void AES_ECB_encrypt(const uint8_t* input, const uint8_t* key, uint8_t *output, int keySize)
{
  switch(keySize)
  {
  case 192:
...
int AES_CBC_encrypt_buffer(uint8_t* output, uint8_t* input, uint32_t length, const uint8_t* key, const uint8_t* iv, int keySize)
{
  uintptr_t i;
