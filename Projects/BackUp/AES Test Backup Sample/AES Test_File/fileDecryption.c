...
static int decrypt_cbc(const char* fileName)
{
  ...
  retval = fread(buf_in, elemSize, 16, fp_in);
  AES_CBC_decrypt_buffer(buf_out, buf_in, retval, key, iv, 128);
  memcpy(&fileSize, buf_out, sizeof(fileSize));
  ...
  while((retval = fread(buf_in, elemSize, MAXBUF, fp_in)) != 0)
  {
    ...
    fwrite(buf_out, elemSize, fileSize < (readedSize + retval) ? fileSize - readedSize : retval, fp_out);
    readedSize += retval;
