...
type
  ...
  private
    ...
    function InternalFindFunctionLines(Lines: TStrings; const CommandName,
      FunctionMacroName: string): Integer;
    function InternalFindFunction(const CommandName, FunctionMacroName: string): Integer;
    function HaveValidFileName(const S: string): Boolean;
  protected
...
const
  StrDelim = [#39, '"', '`'];
  ComentChars = [';', '#'];
  sTempScriptExt = '.hmnetempscript%d';
...
var
  FSearchedFileNames: TStringList;
...
const
  sNSISDir = '${NSISDIR}';
begin
  if AnsiPos(sNSISDir, IncludeFileName) > 0 then
    Result := StringReplace(IncludeFileName, sNSISDir,
      ExtractFileDir(MainFrm.CurCompilerProfile.Compiler), [rfReplaceAll])
  else begin
...
begin
  try
    ...
    if FEditor.FTempScriptExt <> '' then
      DeleteFile(FEditor.FileName + FEditor.FTempScriptExt);
...
procedure TEditFrm.ShowScriptErrorLine(ErrorFileName: String;
  ErrorLineNumber: Integer);
begin
  if ExtractFileName(ErrorFileName) = FTempScriptExt then
    ErrorFileName := '' else
  if ExtractFileExt(ErrorFileName) = FTempScriptExt then
...
procedure TEditFrm.Compilar(Run: Boolean = False);
  ...
  var
    ...
    begin
      ...
      if OptionsIni.ReadBool('Compiler', 'UseNotifyWindow', True) then
    ...
    begin
      ...
      begin
        ...
        repeat
          Inc(I);
          FTempScriptExt := Format(sTempScriptExt, [I]);
          if FileName = '' then
            Result := IncludeTrailingBackslash(TempPath) + FTempScriptExt
          else 
            Result := FileName + FTempScriptExt;
  ...
  with Msg.CopyDataStruct^ do
    case dwData of
      ...
      MAKENSIS_NOTIFY_OUTPUT:
       SetString(FExeName, PChar(lpData), cbData - 1);
    end;
end;
...
{  Finds a function or macro }
  ...
  function FindFunctionInFile(const FileName: string): Integer;
  var
    aLines: TStrings;
    Form: TCustomForm;
  begin
    ...
    if FSearchedFileNames = nil then
    begin
      FSearchedFileNames := TStringList.Create;
      FSearchedFileNames.Sorted := True;
    end;

    if FSearchedFileNames.IndexOf(FileName) >= 0 then
      Exit;

    FSearchedFileNames.Add(FileName);
...
begin
  ...
  // Last find in the NSIS configuration file
    ...
    begin
      I := FindFunctionInFile(ExtractFilePath(Compiler) +  'nsisconf.nsh');
      if I > 0 then
      begin
        Result := -I;
        Exit;
...
begin
  Screen.Cursor := crHourGlass;
  try
    FSearchedFileNames := nil;
    try
      Result := InternalFindFunction(Cmd, Param);
    finally
      FreeAndNil(FSearchedFileNames);
    end;
  finally
    Screen.Cursor := crDefault;
  end;
...
begin
  ...
  TSynEditAccess(LogBox).SetSelTextPrimitiveEx(smNormal, PChar(Msg.WParam), False);
...
begin
  ...
  begin
    ...
    if (Wfn <> '') then
    begin
      if ExtractFileExt(Wfn) = FTempScriptExt then
...
function TEditFrm.HaveValidFileName(const S: string): Boolean;
var
  Wfn: string;
begin
  Result := False;
...
begin
  ...
  (AnsiPos(SConfigError, Line) > 0)  or (AnsiPos(SWarning, Trim(Line)) = 1) or
    (AnsiPos(SErrorInMacro, Line) > 0) or HaveValidFileName(Line) then
    aCursor := crHandPoint;
end;
...
begin
  ...
  Special := (AnsiPos(SIncError, aLine) > 0)  or (AnsiPos(SError, aLine) > 0)  or
    (AnsiPos(SConfigError, aLine) > 0)  or (AnsiPos(SWarning, Trim(aLine)) = 1) or
      (AnsiPos(SErrorInMacro, aLine) > 0) or HaveValidFileName(aLine);
  if Special then
    FG := clRed;
end;
